...
output_1 = "output1.parquet.gzip"

data_source2 = "Datasource\source 2"
output_2 = "output2.parquet.gzip"

record_ignore =  "Record.txt"
...
# Get the directory path containing the executable -- so we wont be needing to declaire or input the source per user
    ...
    existing_output_path = os.path.join(executable_directory, output_source, output_1)
    ...
    record_file_path = os.path.join(executable_directory,output_source,record_ignore)
    ...
    for file in files:
        ...
        try: 
            ...
            # sheet_name = "Sheet1" ,  engine = "pyxlsb"
...
# Get the directory path containing the executable -- so we wont be needing to declaire or input the source per user
    # executable_path = os.path.abspath(sys.argv[0])
    # executable_directory = os.path.dirname(executable_path)
    print("\Path check 2= ",executable_directory) # check if will return correct path

    # Change the current working directory to 'Datasource' folder inside the directory of the executable
    os.chdir(os.path.join(executable_directory, data_source2))

    # Read the existing OUTPUT.csv file if it exists
    existing_output_path = os.path.join(executable_directory, output_source, output_2)
    if os.path.exists(existing_output_path):
        df_con = pd.read_parquet(existing_output_path)
    else:
        df_con = pd.DataFrame()
